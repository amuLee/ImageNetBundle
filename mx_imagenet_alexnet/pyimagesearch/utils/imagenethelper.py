...
class ImageNetHelper:
    ...
    def buildClassLabels(self):
        ...
        # to integers, then initialize the label mapping dictionary
    ...
    def buildTrainingSet(self):
        ...
        rows = open(self.config.TRAIN_LISTA).read().strip()
        rows = rows.split("\n")
        paths = []
        labels = []
        ...
        for row in rows:
            ...
            # constuct the full path to the training image, then 
    ...
    def buildValidationSet(self):
        ...
        for (row, label) in zip(valFilenames, valLabels):
            ...
            # constuct the full path to the validation image, then 
